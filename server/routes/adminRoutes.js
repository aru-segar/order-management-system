  ...
  (req, res) => {
    ...
    router.get("/pizzas", verifyToken, requireRole("owner"), (req, res) => {
      db.query("SELECT * FROM pizza_types", (err, rows) => {
        if (err)
          return res.status(500).json({ error: "Error fetching pizza types" });
        res.json(rows);
      });
    });
...
router.get('/analytics/stock-levels', verifyToken, requireRole('owner'), (req, res) => {
  const query = `
    SELECT name, stock_quantity FROM ingredients
  `;
  ...
  db.query(query, (err, rows) => {
    ...
    router.post("/pizzas", verifyToken, requireRole("owner"), (req, res) => {
      ...
      db.query(
        ...
        (err, result) => {
          if (err)
            return res.status(500).json({ error: "Pizza insert failed" });
          ...
          db.query(
            ...
            (err) => {
              if (err)
                return res
                  .status(500)
                  .json({ error: "Ingredient mapping failed" });
