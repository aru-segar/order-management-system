...
router.post("/place", verifyToken, requireRole("customer"), (req, res) => {
  ...
  db.query(ingredientQuery, [pizzaIds], (err, ingredients) => {
    ...
    for (const item of items) {
      for (const ing of ingredients.filter(
        (i) => i.pizza_type_id === item.pizzaTypeId
      )) {
        const needed = ing.quantity_required * item.quantity;
        if (!required[ing.ingredient_id]) required[ing.ingredient_id] = 0;
        required[ing.ingredient_id] += needed;
    ...
    // 2. Proceed with order insertion
    ...
    db.query(insertOrderQuery, [customerId, referenceId], (err, result) => {
      ...
      // Insert order items
      const orderItemsQuery = `INSERT INTO order_items (order_id, pizza_type_id, quantity) VALUES ?`;
      const values = items.map((i) => [orderId, i.pizzaTypeId, i.quantity]);
      db.query(orderItemsQuery, [values]);
      ...
      res.json({ message: "Order placed!", referenceId });
    });

    // Get order status and details by reference ID
    ...
    WHERE o.reference_id = ?`;
      ...
      db.query(orderQuery, [refId], (err, rows) => {
